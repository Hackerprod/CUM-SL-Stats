...
using static SKYNET_Button;

public class SKYNET_AlertBox : Control
{
    [Flags]
    ...
    private Color CustomizedColor;

    private Color CustomizedText;
    ...
    public void ShowControl(NotificationType Kind, string Str, int Interval)
    {
        K = Kind;
        Text = Str;
        Visible = true;
    ...
    public SKYNET_AlertBox()
    {
        State = MouseState.None;

        SuccessColor = Color.FromArgb(18, 104, 48);
        SuccessText = Color.FromArgb(115, 250, 190);

        CustomizedColor = Color.FromArgb(43, 47, 48); //Color del Panel  
        CustomizedText = Color.FromArgb(7, 164, 245); //Color del texto

        ErrorColor = Color.FromArgb(43, 47, 48);
        ErrorText = Color.Red;
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        checked
        {
            ...
            switch (K)
            {
                ...
                case NotificationType.Personalizado:
                    {
                        g.FillRectangle(new SolidBrush(CustomizedColor), rect); //Color del panel
                        g.FillEllipse(new SolidBrush(CustomizedText), new Rectangle(8, 9, 24, 24)); //Color del icono
                        g.FillEllipse(new SolidBrush(CustomizedColor), new Rectangle(10, 11, 20, 20)); //Color del icono
                        g.DrawString("ü", new Font("Wingdings", 22f), new SolidBrush(CustomizedText), new Rectangle(7, 7, W, H), Helpers.NearSF); //Color del icono
                        g.DrawString(Text, Font, new SolidBrush(CustomizedText), new Rectangle(48, 12, W, H), Helpers.NearSF); //Color del Texto
                        g.FillEllipse(new SolidBrush(Color.FromArgb(57, 62, 63)), new Rectangle(W - 30, H - 29, 17, 17)); //Circulito de cerrar
                        g.DrawString("r", new Font("Marlett", 8f), new SolidBrush(CustomizedColor), new Rectangle(W - 28, 16, W, H), Helpers.NearSF); //Crucecita de cerrar
                        MouseState state2 = State;
                        if (state2 == MouseState.Over)
                        {
