...
namespace SQLite.Net.Platform.WinRT
{
    public class SQLiteApiWinRT : ISQLiteApiExt
    {
        ...
        public SQLiteApiWinRT(string tempFolderPath = null, bool useWinSqlite = false)
        {
            ...
            if (_useWinSqlite)
            {
                WinSQLite3.SetDirectory(/*temp directory type*/2, tempFolderPath ?? Path.Combine(Common.GetPath(), "Data"));
            }
            else
            {
                SQLite3.SetDirectory(/*temp directory type*/2, tempFolderPath ?? Path.Combine(Common.GetPath(), "Data"));
            }
        }
